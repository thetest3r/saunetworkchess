...
namespace NetworkChess
{
    ...
    public class Game1 : Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            Animation pieceAnimation = new Animation();
            Texture2D pieceTexture = Content.Load<Texture2D>("Graphics\\white_pawn");
            pieceAnimation.Initialize(pieceTexture, Vector2.Zero, 115, 69, 8, 30, Color.White, 1f, true);
            Vector2 piecePosition = new Vector2(GraphicsDevice.Viewport.TitleSafeArea.X, GraphicsDevice.Viewport.TitleSafeArea.Y +
                GraphicsDevice.Viewport.TitleSafeArea.Height / 2);
            white_pawn.Initialize(pieceAnimation, piecePosition);
        ...
        private void UpdatePlayer(GameTime gameTime)
        {
            ...
            while (TouchPanel.IsGestureAvailable)
            {
                ...
                {
                    white_pawn.Position += gesture.Delta;
                }
            }
            // Get Mouse state and then capture the button type and respond button press
            Vector2 mousePosition = new Vector2(currentMouseState.X, currentMouseState.Y);
            if (currentMouseState.LeftButton == ButtonState.Pressed)
            {
                Vector2 posDelta = mousePosition - white_pawn.Position;
                posDelta.Normalize();
                posDelta = (posDelta * playerMoveSpeed);
                white_pawn.Position = white_pawn.Position + posDelta;
            }
